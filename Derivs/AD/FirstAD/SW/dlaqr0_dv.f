...
C
C  Differentiation of dlaqr0 in forward (tangent) mode: (multi-directional mode)
C   variations  of output variables: wr h z work wi
C   with respect to input variables: wr h z work wi
      SUBROUTINE DLAQR0_DV(wantt, wantz, n, ilo, ihi, h, hd, ldh, wr,
...
C
     ...
     +                 swapd(nbdirsmax)
      CHARACTER*2 jbcmpz
      INTEGER arg1, arg2, i, ii1, ii2, ILAENV, inf, it, itmax, k, kacc22
...
C     =======
C
C     DLAQR0 computes the eigenvalues of a Hessenberg matrix H
...
C
C           If LWORK = -1, then DLAQR0 does a workspace query.
C           In this case, DLAQR0 checks the input parameters and
...
C             =  0:  successful exit
C           .GT. 0:  if INFO = i, DLAQR0 failed to compute all of
...
C        ==== Estimate optimal workspace ====
C
C        ==== Workspace query call to DLAQR3 ====
C
          arg1 = nwr + 1
          arg2 = -1
          CALL DLAQR3_DV(wantt, wantz, n, ilo, ihi, arg1, h, hd, ldh,
...
C           ==== Aggressive early deflation ====
C
                CALL DLAQR3_DV(wantt, wantz, n, ktop, kbot, nw, h, hd,
...
C              .    DLAQR3 above or from the eigenvalues
...
C
     ...
     +                               nbdirs)
                  ...
                  ELSE
C
C                 ==== Got NS/2 or fewer shifts? Use DLAQR4 or
C                 .    DLAHQR on a trailing principal submatrix to
...
C
     ...
     +                               ldh, nbdirs)
                      IF (ns .GT. nmin) THEN
                        CALL DLAQR4_DV(.false., .false., ns, 1, ns, h(kt
     +                                 , 1), hd(1, kt, 1), ldh, wr(ks),
     +                                 wrd(1, ks), wi(ks), wid(1, ks), 1
     +                                 , 1, zdum, zdumd, 1, work, workd
     +                                 , lwork, inf, nbdirs)
                      ELSE
                        CALL DLAHQR_DV(.false., .false., ns, 1, ns, h(kt
     +                                 , 1), hd(1, kt, 1), ldh, wr(ks),
     +                                 wrd(1, ks), wi(ks), wid(1, ks), 1
     +                                 , 1, zdum, zdumd, 1, inf, nbdirs)
                      END IF
                      ks = ks + inf
C
