...
DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: hpart, hcopy
INTEGER status
INTEGER lleft,rleft
...
DO
   IF ( ANY(zerorows) .AND. (iq .LE. qrows) ) THEN
      ...
      ALLOCATE(hpart(nz,3*neq),hcopy(nz,3*neq), STAT=status)
      hpart(:,:) = 0.0d0
      hcopy = h(selector,:)
      CALL shiftright(hcopy,nz,3*neq,neq,hpart)
      h(selector,:) = hpart
      DEALLOCATE(hpart,hcopy,STAT=status)
